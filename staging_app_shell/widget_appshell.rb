...
#Customers have to select the branch first
    ...
    checkbox = @driver.find_element(:id, "merchant_profile_key_account_attributes_merchant_picker_in_booking_service");
        if checkbox.selected?
    ...
    sleep 3
    @driver.find_element(:id, "merchant_profile_key_account_attributes_merchant_picker_in_booking_service").click
    @driver.find_element(:css, "input.btn-blue").click
    verify { assert_include @driver.find_element(:css, "div.code-snippet-preview").text, "true" }

#Inform customers about newsletter subscription on
    @driver.get "https://staging.shore.com/merchant/widget?appshell=true"
    checkbox = @driver.find_element(:id, "merchant_profile_key_account_attributes_newsletter_terms");
      if checkbox.selected?
        @driver.find_element(:css, "input.btn-blue").click
      else
        checkbox.click
